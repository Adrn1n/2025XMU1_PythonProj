...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "-p", "--pages", type=int, default=1, help="Number of pages to scrape (default: 1)"
    )
    parser.add_argument(
        "-o", "--output", type=str, help="Output file path (default: auto-generated)"
    )
    parser.add_argument(
        "--no-save-results", action="store_true", help="Do not save search results to file"
    )
    parser.add_argument(
    ...
    parser.add_argument("--no-log-file", action="store_true", help="Do not write logs to file")
    parser.add_argument(
        "--concurrent",
        type=int,
    ...
    parser.add_argument("--proxy", action="store_true", help="Use proxy for all requests")
    ...
    try:
        ...
        try:
            ...
            if not args.no_cache and cache_file.exists():
                logger.warning("【MAIN】Attempting to load partial results from cache...")
                # Add logic to recover from cache if needed
            results = []
        except Exception as e:
            logger.error(f"【MAIN】Unknown error occurred during search: {str(e)}", exc_info=True)
            results = []
        ...
        if logging.getLogger().hasHandlers():
            logging.getLogger().error(f"【MAIN】Error during execution: {str(e)}", exc_info=True)
