...
class SearchHandler:
    ...
    async def perform_search_and_generate(
        ...
        try:
            ...
            from ollama.ollama_integrate import OptimizedOllamaIntegrate, SearchConfig as IntegrateSearchConfig, OllamaConfig
...
class ChatHandler:
    ...
    async def _stream_text_chunks(
        ...
        for i in range(0, len(words), chunk_size):
            ...
            content = (" " if i > 0 and start_with_space else "") + " ".join(chunk_words)
    ...
    async def create_streaming_chunks(
        ...
        try:
            ...
            async for chunk in self._stream_text_chunks(cleaned_text, model, completion_id, start_with_space=False):
                yield chunk
        ...
        try:
            ...
            try:
                result = await self.search_handler.perform_search_and_generate(
                    model=model, query=query, temperature=temperature,
                ...
                completion_chunk = self.compatibility_utils.create_chat_completion_chunk(
                    model=model,
                    delta_content="\n\n✅ Generating response...\n\n",
                    message_id=completion_id
                )
                ...
                # Stream response
                async for chunk in self._stream_text_chunks(response_text, model, completion_id):
                    yield chunk
                ...
                try:
                    fallback = await self.generate_direct_response(model, query, temperature, top_p, top_k, max_tokens, True)
                    async for chunk in self._stream_text_chunks(fallback, model, completion_id):
                        yield chunk
                        
                except (RuntimeError, ValueError, ConnectionError, TimeoutError) as fallback_error:
            ...
            if final_chunk and final_chunk.startswith('{"id":'):
                ...
                logger.error(f"Invalid final chunk in search streaming: {repr(final_chunk)}")
    ...
    async def process_chat_completion(
        ...
        if use_search:
            ...
            if stream:
                ...
                usage = ChatHandler.calculate_token_usage(actual_question, response_text)
